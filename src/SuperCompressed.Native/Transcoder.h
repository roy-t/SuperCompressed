...
extern "C"
{
	enum Format : uint8_t
	{
		...
		PVRTC1_4_RGBA = 9,					// Opaque+alpha, most useful for simple opacity maps. If .basis file doesn't have alpha PVRTC1_4_RGB will be used instead. Lowest quality of any supported texture format.
    ...
    _declspec(dllexport) void InitializeTranscoder();

    _declspec(dllexport) int32_t GetImageCount(uint8_t* data, int32_t length);

    _declspec(dllexport) int32_t GetLevelCount(uint8_t* data, int32_t length, int32_t imageIndex);
